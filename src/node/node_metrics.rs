...
impl NodeMetrics {
    ...
    pub fn start_server(&self, addr: &str) -> Result<()> {
        ...
        let socket_addr = match addr.parse() {
            Ok(addr) => addr,
            Err(e) => return Err(BlockchainError::Other(format!("Invalid metrics address: {}", e))),
        };
        ...
        match self.service.start_server() {
            Ok(_) => Ok(()),
            Err(e) => Err(BlockchainError::Other(format!("Failed to start metrics server: {}", e))),
        }
    }
    ...
    pub fn register_transaction_time(&self, time_seconds: f64) {
        self.service.observe_histogram("transaction_processing_time", time_seconds);
    }
