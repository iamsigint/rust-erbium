...
impl NodeManager {
    ...
    /// Start the JSON-RPC 2.0 API server (replaces REST API)
    async fn start_json_rpc_api(&mut self) -> Result<()> {
        log::warn!("JSON-RPC 2.0 API server not fully implemented yet - using placeholder");
        log::info!("RPC methods will be available through modular system when integrated");

        // TODO: Implement proper RPC server when axum/tower dependencies are added
        // For now, this is a placeholder that will be replaced with:
        // - axum server with crate::rpc::router()
    ...
    /// Legacy RPC placeholder - remove when fully migrated
    async fn start_rpc_api(&mut self) -> Result<()> {
        // This was the old RPC API, now we use JSON-RPC through start_json_rpc_api()
        log::debug!("RPC API legacy method - using JSON-RPC instead");
        Ok(())
    }
