...
impl SignatureManager {
    pub fn new() -> Self {
        ...
        schemes.insert(
            ...
            Box::new(DilithiumScheme::new(SignatureScheme::Dilithium2)) as Box<dyn SignatureSchemeTrait>
        );
    ...
    pub fn create_signature(
        ...
        let signer = self.schemes.get(&scheme)
    ...
    pub fn verify_signature(&self, signature: &Signature, message: &[u8]) -> Result<bool> {
        let verifier = self.schemes.get(&signature.scheme)
            .ok_or_else(|| BlockchainError::Crypto("Unsupported signature scheme".to_string()))?;
    ...
    fn derive_public_key(&self, private_key: &[u8], scheme: &SignatureScheme) -> Result<Vec<u8>> {
        ...
        match scheme {
            ...
            SignatureScheme::Dilithium3 => {
                Ok(private_key[..1952.min(private_key.len())].to_vec())
            }
            SignatureScheme::Dilithium5 => {
                Ok(private_key[..2592.min(private_key.len())].to_vec())
            }
        }
    }
}
...
impl Signature {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        if bytes.len() < 1 + 4 + 4 {
            return Err(BlockchainError::Crypto("Invalid signature bytes".to_string()));
        }
        ...
        let scheme = match bytes[offset] {
            ...
            _ => return Err(BlockchainError::Crypto("Unknown signature scheme".to_string())),
        };
        offset += 1;
        ...
        if bytes.len() < offset + pubkey_len + 8 {
            return Err(BlockchainError::Crypto("Invalid signature bytes length".to_string()));
        }
