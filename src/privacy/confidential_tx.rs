...
impl ConfidentialTxBuilder {
    ...
    pub fn create_confidential_transfer(
        ...
        for (i, b) in recipient_bytes.iter().take(32).enumerate() { key_bytes[i] = *b; }
        let recipient_pk = CompressedRistretto(key_bytes);
        let (_stealth_address, _ephemeral_key) = self.stealth_system.generate_stealth_address(
            &recipient_pk,
            &recipient_pk,
        )?;
        ...
        let amount_range_proof = self.range_prover.generate_range_proof(amount, &amount_blinding, 64)?;
        let fee_range_proof = self.range_prover.generate_range_proof(fee, &fee_blinding, 32)?;
        
        // Create base transaction
        // For compilation, keep original recipient address in base transaction
        let base_tx = Transaction::new_transfer(
            from,
            to.clone(),
            amount,
            fee,
        ...
        let confidential_tx = ConfidentialTransaction {
            ...
            input_commitments: vec![amount_commitment],
            output_commitments: vec![fee_commitment],
    ...
    fn create_binding_signature(
        ...
        transcript.append_message(b"tx_hash", transaction.hash().as_bytes());
        transcript.append_message(b"amount_commitment", amount_commitment.as_bytes());
        transcript.append_message(b"fee_commitment", fee_commitment.as_bytes());
    ...
    pub fn verify_confidential_transaction(
        ...
        // Verify range proofs
        for (i, (commitment, proof)) in confidential_tx.output_commitments
            .iter()
            .zip(confidential_tx.range_proofs.iter())
            .enumerate() 
        {
            let bit_length = if i == 0 { 64 } else { 32 }; // Amount vs fee
            if !self.range_prover.verify_range_proof(
                proof,
                commitment.as_bytes(),
                bit_length,
            )? {
    ...
    fn verify_binding_signature(&self, confidential_tx: &ConfidentialTransaction) -> Result<bool> {
        ...
        transcript.append_message(b"tx_hash", confidential_tx.base_transaction.hash().as_bytes());
        ...
        // In real implementation, we would get the public key from the transaction
        let public_key = vec![]; // Placeholder
        
        use crate::crypto::dilithium::Dilithium;
        Dilithium::verify(&public_key, &challenge, &confidential_tx.binding_signature)
    }
